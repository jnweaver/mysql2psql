...
class Mysql2psql
  ...
  class PostgresDbWriter < PostgresWriter
    ...
    def truncate(table)
      truncate_sql = "TRUNCATE #{PGconn.quote_ident(table.name)} CASCADE"
      @conn.exec(truncate_sql)

      table.columns.select{ |c| c[:auto_increment] }.map do |column|
        serial_key = column[:name]
        maxval = column[:maxval].to_i < 1 ? 1 : column[:maxval].to_i + 1
        serial_sql = "SELECT pg_catalog.setval(pg_get_serial_sequence('#{table.name}', '#{serial_key}'), #{maxval}, true)"
        @conn.exec(serial_sql)
      end
    end
